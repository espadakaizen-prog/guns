...
class DiscordAuth {
    ...
    /**
     * Generate Discord OAuth login URL (uses implicit grant - works on static hosting)
     */
    getLoginUrl() {
    ...
    /**
     * Handle OAuth callback - supports both implicit grant (hash fragment) and authorization code (query param)
     */
    async handleCallback() {
        // Check for implicit grant token in URL hash (e.g., #access_token=xxx&token_type=Bearer)
        ...
        // Fallback: Check for authorization code in query params (for backend server mode)
        const params = new URLSearchParams(window.location.search);
        const code = params.get('code');

        if (!code) {
            console.log('No authorization token or code found');
            return false;
        }

        try {
            // Exchange code for access token (only works with backend server)
